...
import { AUTH_KEY, loadStoredUser, setAccountPassword, setStoredUser } from "@/lib/user-storage";
...
const LoginPage = () => {
  ...
  async function handleLogin(formData: FormData) {
    ...
    const sameAccount = prev?.email === formData.email;
    setStoredUser({
      name: sameAccount && prev?.name ? prev.name : formData.email.split("@")[0],
      email: formData.email,
      avatar: sameAccount && prev?.avatar != null ? prev.avatar : formData.email.slice(0, 2).toUpperCase(),
      bio: sameAccount ? prev?.bio : undefined,
      memberSince: sameAccount && prev?.memberSince ? prev.memberSince : new Date().toISOString(),
    });
