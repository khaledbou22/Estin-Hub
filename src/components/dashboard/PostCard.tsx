...
export function PostCard({ post }: PostCardProps) {
  ...
  return (
    <Card
      className={`post-card-hover group flex h-full flex-col gap-0 rounded-[14px] border border-[#E8ECEF] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.05)] dark:border-[#2A2A2A] dark:bg-[#101010] ${cardHoverClass}`}
    >
      <CardHeader className="px-5 pb-0 pt-[18px]">
        <div className="mb-[10px] flex items-center justify-between">
          <span className="category-badge-hover">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={`h-7 rounded-[7px] px-2.5 text-[11px] font-semibold ${
                saved
                  ? "border-[#6C63FF] text-[#6C63FF] bg-[#EEF2FF]"
        ...
        <div className="mb-[12px] text-[12px] text-[#94A3B8]">
          ...
          {post.category === "marketplace" && post.condition && (
            <div className="flex items-center gap-2">
              ...
              <p>Condition: {conditionLabel(post.condition)}</p>
            </div>
          )}
          {post.category === "services" && <p>Type: {post.serviceType ?? "Service"}</p>}
      ...
      <div className="px-5 pb-[18px]">
        <div className="mb-2.5 flex items-center justify-between gap-2">
          <Link
            ...
            className="group/author flex min-w-0 items-center gap-[6px] hover:opacity-90"
          >
            <UserAvatar user={post.author} className="h-[30px] w-[30px]" />
            <span className="truncate text-[13px] font-medium text-[#374151] transition-colors duration-150 group-hover/author:text-[#6C63FF] dark:text-[#E5E7EB]">
              {post.author.name}
            </span>
          </Link>
        ...
        <div className="grid grid-cols-2 gap-[6px]">
          ...
          <Button
            ...
            className={`btn-action-hover h-7 rounded-[7px] px-3 text-[12px] transition-all duration-150 ${secondaryActionClass}`}
          >
      ...
      <Dialog open={contactOpen} onOpenChange={setContactOpen}>
        <DialogContent className="border-[#E8ECEF] sm:max-w-md dark:border-[#2A2A2A] dark:bg-[#101010]">
          <DialogHeader>
            <DialogTitle className="text-[#0F172A] dark:text-[#F8FAFC]">
              Contact Information
            </DialogTitle>
            <DialogDescription>
